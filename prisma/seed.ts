...
async function main() {
  console.log('🌱 Démarrage du seed TogoTransit (Données réelles du Togo pour la soutenance)...')

  // Nettoyage préalable dans l'ordre des contraintes
  ...
  console.log(`✅ ${villesMap.size} Villes togolaises créées.`)

  // 3. Compagnies réelles de transport au Togo
  ...
  console.log('✅ 4 Compagnies majeures créées (Nagodé, SOLIM, LK Transport, Rakiéta).')

  // 4. Agences locales physiques
  const agencesData = [
  ...
  console.log(`✅ ${agencesData.length} Agences locales réparties au Togo.`)

  // 5. Flotte de véhicules
  const nagodeBus1 = await prisma.vehicule.create({
    data: {
      compagnie_id: nagode.id,
      immatriculation: 'TG-8421-BF',
      type: 'Autocar VIP Yutong Climatisé (55 places)',
      nombre_places: 55,
      statut: 'disponible',
    },
  })
  const nagodeBus2 = await prisma.vehicule.create({
    data: {
      compagnie_id: nagode.id,
      immatriculation: 'TG-6312-AH',
      type: 'Autocar Standard King Long (50 places)',
      nombre_places: 50,
      statut: 'disponible',
    },
  })
  ...
  const solimBus1 = await prisma.vehicule.create({
    data: {
      compagnie_id: solim.id,
      immatriculation: 'TG-5198-AL',
      type: 'Bus Confort Toyota Coaster (30 places)',
      nombre_places: 30,
      statut: 'disponible',
    },
  })
  const solimBus2 = await prisma.vehicule.create({
    data: {
      compagnie_id: solim.id,
      immatriculation: 'TG-7724-BM',
      type: 'Autocar Grand Tourisme (55 places)',
      nombre_places: 55,
      statut: 'disponible',
    },
  })
  ...
  const lkMinibus = await prisma.vehicule.create({
    data: {
      compagnie_id: lkTransport.id,
      immatriculation: 'TG-3410-AP',
      type: 'Minibus Express Toyota HiAce (18 places)',
      nombre_places: 18,
      statut: 'disponible',
    },
  })

  console.log('✅ Flotte de véhicules enregistrée avec plaques togolaises.')

  // 6. Utilisateurs & Comptes de Démonstration (Soutenance)
  const superAdmin = await prisma.utilisateur.create({
    data: {
      nom: 'Edoh',
      prenom: 'Komi (Super Admin)',
      email: 'superadmin@togotransit.tg',
  ...
  const adminNagode = await prisma.utilisateur.create({
    data: {
      nom: 'Houndjo',
      prenom: 'Nayra (Gestionnaire)',
      email: 'admin@nagode.tg',
      telephone: '+228 90 11 22 33',
      mot_de_passe: await bcrypt.hash('Nagode2026!', 10),
      role: 'gestionnaire',
      compagnie_id: nagode.id,
      statut: 'actif',
    },
  ...
  const adminSolim = await prisma.utilisateur.create({
    data: {
      nom: 'Anakpa',
      prenom: 'Michel (Gestionnaire)',
      email: 'admin@solim.tg',
      telephone: '+228 91 22 33 44',
      mot_de_passe: await bcrypt.hash('Solim2026!', 10),
      role: 'gestionnaire',
      compagnie_id: solim.id,
      statut: 'actif',
    },
  ...
  const adminLk = await prisma.utilisateur.create({
    data: {
      nom: 'N\'djelle',
      prenom: 'Marcel (Gestionnaire)',
      email: 'admin@lk.tg',
      telephone: '+228 92 33 44 55',
      mot_de_passe: await bcrypt.hash('Lk2026!', 10),
      role: 'gestionnaire',
      compagnie_id: lkTransport.id,
      statut: 'actif',
    },
  ...
  const chauffeurNagode = await prisma.utilisateur.create({
    data: {
      nom: 'Lawson',
      prenom: 'Koffi (Chauffeur)',
      email: 'chauffeur@nagode.tg',
      telephone: '+228 99 88 77 66',
      mot_de_passe: await bcrypt.hash('Chauffeur2026!', 10),
      role: 'voyageur',
      compagnie_id: nagode.id,
      statut: 'actif',
    },
  ...
  const clientTest = await prisma.utilisateur.create({
    data: {
      nom: 'Abalo',
      prenom: 'Kossi (Client)',
      email: 'voyageur@gmail.com',
      telephone: '+228 90 55 66 77',
      mot_de_passe: await bcrypt.hash('Voyageur2026!', 10),
      role: 'voyageur',
      statut: 'actif',
    },
  })

  console.log('✅ Comptes démo créés (Super Admin, Gestionnaires Nagodé/SOLIM/LK, Voyageur).')

  // 7. Tarifs de fret / colis
  const faresData = [
  ...
  console.log(`✅ ${faresData.length} Grilles tarifaires de fret enregistrées.`)

  // 8. Trajets programmés réels (Multi-compagnies pour le comparateur)
  const now = new Date()
  ...
  // Trajets Lomé -> Kara (Nagodé vs SOLIM)
  const trajetNagodeLomeKara = await prisma.trajet.create({
    data: {
      compagnie_id: nagode.id,
      vehicule_id: nagodeBus1.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Kara').id,
      date_depart: makeDate(tomorrow, 6, 30),
      heure_depart: makeDate(tomorrow, 6, 30),
      prix: 8500,
      places_disponibles: 52,
      statut: 'planifie',
      driver_id: chauffeurNagode.id,
      currentLat: 6.1375,
      currentLng: 1.2123,
    },
  })

  const trajetSolimLomeKara = await prisma.trajet.create({
    data: {
      compagnie_id: solim.id,
      vehicule_id: solimBus2.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Kara').id,
      date_depart: makeDate(tomorrow, 7, 0),
      heure_depart: makeDate(tomorrow, 7, 0),
      prix: 9000,
      places_disponibles: 48,
      statut: 'planifie',
      currentLat: 6.1375,
      currentLng: 1.2123,
    },
  ...
  // Trajets Lomé -> Kpalimé (LK Transport vs SOLIM)
  const trajetLkLomeKpalime = await prisma.trajet.create({
    data: {
      compagnie_id: lkTransport.id,
      vehicule_id: lkMinibus.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Kpalimé').id,
      date_depart: makeDate(tomorrow, 8, 0),
      heure_depart: makeDate(tomorrow, 8, 0),
      prix: 2500,
  ...
  const trajetSolimLomeKpalime = await prisma.trajet.create({
    data: {
      compagnie_id: solim.id,
      vehicule_id: solimBus1.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Kpalimé').id,
      date_depart: makeDate(tomorrow, 9, 30),
      heure_depart: makeDate(tomorrow, 9, 30),
      prix: 2800,
      places_disponibles: 26,
      statut: 'planifie',
    },
  })

  // Trajets Lomé -> Sokodé
  const trajetSolimLomeSokode = await prisma.trajet.create({
    data: {
      compagnie_id: solim.id,
      vehicule_id: solimBus1.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Sokodé').id,
      date_depart: makeDate(tomorrow, 7, 30),
      heure_depart: makeDate(tomorrow, 7, 30),
      prix: 6500,
  ...
  // Trajets Lomé -> Dapaong / Cinkassé
  const trajetNagodeLomeCinkasse = await prisma.trajet.create({
    data: {
      compagnie_id: nagode.id,
      vehicule_id: nagodeBus2.id,
      ville_depart_id: villesMap.get('Lomé').id,
      ville_arrivee_id: villesMap.get('Cinkassé').id,
      date_depart: makeDate(tomorrow, 5, 30),
      heure_depart: makeDate(tomorrow, 5, 30),
      prix: 13000,
  ...
  console.log('✅ Trajets interurbains multi-compagnies planifiés pour le comparateur.')

  // 9. Réservations & Billets avec QR Code
  const reservation1 = await prisma.reservation.create({
    data: {
      utilisateur_id: clientTest.id,
      trajet_id: trajetNagodeLomeKara.id,
      nombre_places: 2,
      montant_total: 17000,
      statut: 'confirmee',
      passagers: {
      ...
      paiements: {
        create: {
          methode: 'tmoney',
          reference_transaction: 'TMONEY-TG-2026-98412',
          montant: 17000,
          statut: 'reussi',
          date_paiement: now,
        },
      ...
      billets: {
        create: [
          {
            code_qr: 'TGT-NAGODE-LOM-KAR-001',
            numero_billet: 'BIL-2026-0001',
            statut: 'valide',
          },
          {
            code_qr: 'TGT-NAGODE-LOM-KAR-002',
            numero_billet: 'BIL-2026-0002',
            statut: 'valide',
          },
        ],
      },
  ...
  console.log('✅ Réservation avec billets QR Code et paiement TMoney créée.')

  // 10. Colis avec Tracking & POD
  const parcel1 = await prisma.parcel.create({
    data: {
      compagnie_id: nagode.id,
      trackingId: 'TG-KARA-2026-8801',
      senderId: clientTest.id,
      senderName: 'Kossi Abalo',
      senderPhone: '+228 90 55 66 77',
      receiverName: 'Mensa Kodjo',
      receiverPhone: '+228 92 11 00 22',
      weight: 12.5,
  ...
  const parcel2 = await prisma.parcel.create({
    data: {
      compagnie_id: lkTransport.id,
      trackingId: 'TG-KPAL-2026-4402',
      senderId: clientTest.id,
      senderName: 'Amavi Koffi',
      senderPhone: '+228 91 88 77 66',
      receiverName: 'Akouvi Sika',
      receiverPhone: '+228 90 33 22 11',
      weight: 5.0,
      category: 'DOCUMENTS',
  ...
  console.log('✅ Colis avec tracking et preuve de livraison (POD) enregistrés.')
  ...
  await prisma.avis.create({
    data: {
      ...
      trajet_id: trajetNagodeLomeKara.id,
      note: 5,
      commentaire: 'Super voyage avec Nagodé ! Bus climatisé très propre et départ à l\'heure exacte.',
    },
  })

  console.log('🎉 Seed TogoTransit terminé avec succès sur PostgreSQL !')
}
