...
export default async function ParcelDetailsPage({
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      case 'DELIVERED': return 'bg-emerald-500/10 text-emerald-700 border-emerald-500/30'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className={`p-6 rounded-2xl border shadow-md flex flex-col gap-4 ${
            parcel.paymentStatus === 'PAID' ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-error-container/10 border-error-container'
          }`}>
            <div className="flex justify-between items-center">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="w-full h-32 bg-gray-50 border border-outline-variant rounded-lg flex items-center justify-center overflow-hidden">
                    {parcel.pod.signatureUrl ? (
                      ...
                      <span className="text-xs italic text-gray-600">Signature numérisée</span>
                    )}
                  </div>
                </div>
